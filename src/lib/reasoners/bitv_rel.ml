...
module Product = struct
  module type S = sig
    include Domains_intf.ComparableType

    val mul : t -> t -> t
    (** Product multiplication. *)

    type var
    (** The type of variables. A value of type [t] represents a product of
        variables with type [var] associated with a nonnegative power. *)

    val var : var -> t
    (** Injects a variable into a product. *)

    val as_var : t -> var option
    (** If this product is equal to a single variable, return that variable. *)

    val fold : (var -> int -> 'a -> 'a) -> t -> 'a -> 'a
    (** Fold over all the variables in the product and the corresponding
        exponent. *)
  end

  module Make(X : Domains_intf.ComparableType) : S with type var = X.t = struct
    type var = X.t

    type t = { variables : int X.Map.t }

    let pp =
      Fmt.using (fun { variables } -> variables) @@
      Fmt.iter_bindings ~sep:(Fmt.any "*") X.Map.iter @@
      fun ppf (x, k) ->
      if k = 1 then X.pp ppf x
      else Fmt.pf ppf "@[%a^%d@]" X.pp x k

    let equal p1 p2 =
      X.Map.equal Int.equal p1.variables p2.variables

    let compare p1 p2 =
      X.Map.compare Int.compare p1.variables p2.variables

    let hash p =
      X.Map.fold (fun x k acc ->
          23 * acc + X.hash x * Hashtbl.hash (k : int)
        ) p.variables 0

    let var x = { variables = X.Map.singleton x 1 }

    let mul p1 p2 =
      let variables =
        X.Map.union
          (fun _ k1 k2 -> Some (k1 + k2))
          p1.variables p2.variables
      in
      { variables }

    let as_var { variables } =
      try
        let r = ref None in
        X.Map.iter (fun x k ->
            match !r with
            | None when k = 1 -> r := Some x
            | _ -> raise Exit
          ) variables;
        !r
      with Exit -> None

    let fold f { variables } acc =
      X.Map.fold f variables acc
...
module Affine = struct
  module type S = sig
    ...
    val fold :
      constant:(constant -> 'a) ->
      coeff:(monomial -> constant -> 'a -> 'a) ->
      t -> 'a
  end

  module Make(K : Int_like)(X : Domains_intf.ComparableType)
    : S with type var = X.t and type monomial = X.t and type constant = K.t =
  struct
    type var = X.t
    ...
    type t = { const : K.t ; coeffs : K.t X.Map.t }

    let pp ppf { const ; coeffs } =
      if not (K.equal const K.zero) || X.Map.is_empty coeffs then (
        K.pp_print ppf const;
        if not (X.Map.is_empty coeffs) then
          Fmt.pf ppf " +@ ";
      );
      Fmt.pf ppf "%a" (
        Fmt.iter_bindings ~sep:(Fmt.any " +@ ") X.Map.iter @@
        fun ppf (x, k) ->
        if K.equal k K.one then X.pp ppf x
        else Fmt.pf ppf "@[%a*%a@]" K.pp_print k X.pp x
      ) coeffs

    let equal a1 a2 =
      K.equal a1.const a2.const &&
      X.Map.equal K.equal a1.coeffs a2.coeffs

    let compare a1 a2 =
      let c = X.Map.compare K.compare a1.coeffs a2.coeffs in
      if c <> 0 then c else K.compare a1.const a2.const

    let hash a =
      X.Map.fold (fun x k acc ->
          23 * acc + X.hash x * K.hash k
        ) a.coeffs (19 * K.hash a.const)

    let const const = { const ; coeffs = X.Map.empty }

    (* 0 -> None /!\ important for normalization *)
    let simplify k =
      if K.equal k K.zero then None else Some k

    let merge op a1 a2 =
      let coeffs =
        X.Map.merge (fun _ k1 k2 ->
            match k1, k2 with
            | None, None -> assert false
            | Some k, None -> simplify (op k K.zero)
            | None, Some k -> simplify (op K.zero k)
            | Some k1, Some k2 -> simplify (op k1 k2)
          ) a1.coeffs a2.coeffs
      in
      { const = op a1.const a2.const
      ; coeffs }

    let add a1 a2 = merge K.add a1 a2

    let ( + ) = add

    let sub a1 a2 = merge K.sub a1 a2

    let ( - ) = sub

    let map op a =
      { const = op a.const
      ; coeffs = X.Map.filter_map (fun _ k -> simplify (op k)) a.coeffs }

    let neg a = map K.neg a

    let (~-) = neg

    let scale k a = map (K.mul k) a

    let ( *$$ ) a k = scale k a

    let var x = { const = K.zero ; coeffs = X.Map.singleton x K.one }

    let as_var a =
      if K.equal a.const K.zero then
        try
          let r = ref None in
          X.Map.iter (fun x k ->
              match !r with
              | None when K.equal k K.one -> r := Some x
              | _ -> raise Exit
            ) a.coeffs;
          !r
        with Exit -> None
    ...
    let fold ~constant ~coeff { const ; coeffs } =
      X.Map.fold coeff coeffs (constant const)
...
module Polynomial = struct
  module type S = sig
    ...
    module Mon : Product.S with type var = var and type t = monomial

    val ( * ) : t -> t -> t
  end

  module Make(K : Int_like)(X : Domains_intf.ComparableType)
    : S with type var = X.t and type constant = K.t =
  struct
    module Mon = Product.Make(X)
    module Aff = Affine.Make(K)(Mon)

    include Aff

    type var = X.t

    let var x = Aff.var (Mon.var x)

    let as_var p = Option.bind (Aff.as_var p) Mon.as_var

    let mul a1 a2 =
      Aff.fold a2
        ~constant:Aff.(fun c2 -> a1 *$$ c2)
        ~coeff:(fun x2 k2 acc ->
            Aff.fold a1
              ~constant:Aff.(fun c1 -> acc + var x2 *$$ K.mul c1 k2)
              ~coeff:Aff.(fun x1 k1 acc ->
                  acc + var (Mon.mul x1 x2) *$$ K.mul k1 k2))

    let ( * ) = mul
  end
end

module IntPolynomial = Polynomial.Make(Z)(Rel_utils.XComparable)

(* Convert a bit-vector term to a polynomial.

   This is essentially a bit-vector version of [Arith.make]. *)
let rec bv2poly uf t =
  match E.term_view t with
  | { f = Bitv (_, n) ; xs = []; _ } ->
    IntPolynomial.const n
  | { f = Op BVadd ; xs = [ x ; y ] ; _ } ->
    IntPolynomial.(bv2poly uf x + bv2poly uf y)
  | { f = Op BVsub ; xs = [ x ; y ] ; _ } ->
    IntPolynomial.(bv2poly uf x - bv2poly uf y)
  | { f = Op BVmul ; xs = [ x ; y ] ; _ } ->
    IntPolynomial.(bv2poly uf x * bv2poly uf y)
  | { f = Op BVnot ; xs = [ x ] ; _ } ->
    IntPolynomial.(-bv2poly uf x - const Z.one)
  | { f = Op Concat ; xs = [ x ; y ] ; _ } ->
    let sz = match Expr.type_info y with Tbitv n -> n | _ -> assert false in
    IntPolynomial.(bv2poly uf x *$$ Z.(one lsl sz) + bv2poly uf y)
  | { f = Op (Repeat n) ; xs = [ x ] ; _ } ->
    assert (n > 0);
    (* (repeat 1) x = x * 1
       (repeat 2) x = x * (2^sz + 1)
       (repeat 3) x = x * (2^(2sz) + 2^sz + 1)
       ...
    *)
    let sz = match Expr.type_info x with Tbitv n -> n | _ -> assert false in
    let rec loop n acc =
      if  n = 1 then acc
      else loop (n - 1) Z.(acc lsl sz lor one)
    in
    IntPolynomial.(bv2poly uf x *$$ loop n Z.one)
  | { f = Op BVshl ; xs = [ x ; y ] ; _ } -> (
      match Shostak.Bitv.embed (Uf.make uf y) with
      | [ { bv = Cte n ; sz } ] -> (
          match Z.to_int n with
          | n ->
            if n >= sz then
              IntPolynomial.const Z.zero
            else
              IntPolynomial.(bv2poly uf x *$$ Z.(one lsl n))
          | exception Z.Overflow ->
            IntPolynomial.const Z.zero
        )
      | _ ->
        IntPolynomial.var @@ Uf.make uf t
    )
  | _ ->
    IntPolynomial.var @@ Uf.make uf t
...
module Interner = struct
  ...
  module EE = Ephemeron.K1.Make(Expr)

  module P = Shostak.Polynome

  type t =
    { bvasr : (int, P.t) Hashtbl.t H.t
    (** An entry [r -> ofs -> p] in this map indicates that the equality:

        [p = bv2nat(r asr ofs)]

        holds in all contexts. *)
    ; bv2poly : X.r EE.t
    (** An entry [e -> r] in this map indicates that the equality:

        [bv2nat(e) = bv2poly(e) + r * 2^sz]

        where [e] has type [Tbitv sz]. *)
    }

  let create () =
    { bvasr = H.create 17
    ; bv2poly = EE.create 17 }

  let find_asr t r ofs =
  ...
  let find_bv2poly t r =
    ...
    with Not_found ->
      let k = X.term_embed (E.fresh_name Tint) in
      EE.replace t.bv2poly r k;
      k
end

(* Used to make sure that we create the same variables in the main environment
   and the case split environment. *)
...
module BV2Nat = struct
  ...
  *)
  ...
  let find_bv2nat bv_r t =
    match BitvNormalForm.normal_form bv_r with
    | Constant cte ->
      (T.Ints.of_bigint cte, Ex.empty), t
    | Atom (x, ofs) ->
      let ext = Extraction.full ~size:(bitwidth x) in
      let (p, ex), t = find_or_init_ext x ext t in
      (T.Ints.(p +$$ ofs), ex), t
    | Composite (x, ofs) ->
      let (p, ex), t = composite x t in
      (T.Ints.(p +$$ ofs), ex), t
...
module ExplainedComparable(V : Domains_intf.ComparableType) :
  Domains_intf.ComparableType with type t = V.t explained =
struct
  include ExplainedOrdered(V)

  let hash { value = v ; _ } = V.hash v

  let equal { value = v1 ; _ } { value = v2 ; _ } = V.equal v1 v2
...
module EC = ExplainedComparable(struct
    include Constraint

    module Set = Set.Make(Constraint)
    module Map = Map.Make(Constraint)
    module Table = Hashtbl.Make(Constraint)
  end)
...
(* Propagation state
   ...
   TODO(bclement): Generalize this to arbitrary domains. *)
type state =
  { intervals_uf : Interval_domains.Ephemeral.Canon.t
  ; bitlists_uf : Bitlist_domains.Ephemeral.Canon.t
  ; intervals : Interval_domains.Ephemeral.t
  ; bitlists : Bitlist_domains.Ephemeral.t
  ; interval_variables : SX.t MX.t
...
let bitlist_structural_propagator =
  Any_propagator.make (module Rel_utils.XComparable) @@
  fun ({ bitlists_uf = dom ; bitlist_variables = vars ; _ } as st) r ->
  HX.replace st.bitlist_changed r ();
  let open Bitlist_domains.Ephemeral.Canon in
...
let bitlist_constraint_propagator =
  Any_propagator.make (module EC) @@
  fun { bitlists_uf = dom ; _ } { explanation = ex ; value = c } ->
  Propagator.propagate_bitlist ~ex dom c

let interval_structural_propagator =
  Any_propagator.make (module Rel_utils.XComparable) @@
  fun ({ intervals_uf = dom ; interval_variables = vars ; _ } as st) r ->
  HX.replace st.interval_changed r ();
  let open Interval_domains.Ephemeral.Canon in
  let get r = !!(entry dom r) in
  let update r d = update ~ex:Explanation.empty (entry dom r) d in
  if X.is_a_leaf r then
...
let interval_constraint_propagator =
  Any_propagator.make (module EC) @@
  fun { intervals_uf = dom ; _ } { explanation = ex ; value = c } ->
  Propagator.propagate_interval ~ex dom c

let interval_from_bitlist_propagator =
  Any_propagator.make (module Rel_utils.XComparable) @@
  fun { intervals = idom ; bitlists = bdom ; _ } r ->
...
let bitlist_from_interval_propagator =
  Any_propagator.make (module Rel_utils.XComparable) @@
  fun { intervals = idom ; bitlists = bdom ; _ } r ->
...
(* Holds the queues for propagation.

   Currently we use two separate queues for single-domain propagators (i.e.
   propagators that only use one domain) and for multi-domain propagators
   (i.e. propagators that propagate information across different domains,
   e.g. between bitlists and intervals).

   TODO(bclement): generalize to an arbitrary queue combination (and possibly
   infer the queue configuration from the schedule?). *)
type queues =
  { to_simplify : Ex.t HC.t
  ; propagation_queue : Any_propagator.Queue.t
  ; consistency_queue : Any_propagator.Queue.t }

let state uf idom bdom =
  let to_simplify = HC.create 17 in
  let propagation_queue = Any_propagator.Queue.create 17 in
  let consistency_queue = Any_propagator.Queue.create 17 in
  let events
      constraint_propagator
      structural_propagator
      consistency_propagator
    =
    let evt_atomic_change r =
      Any_propagator.Queue.push propagation_queue structural_propagator r;
      Any_propagator.Queue.push consistency_queue consistency_propagator r
    and evt_composite_change r =
      Any_propagator.Queue.push propagation_queue structural_propagator r;
      Any_propagator.Queue.push consistency_queue consistency_propagator r
    and evt_watch_trigger c =
      HC.replace to_simplify c.value c.explanation;
      Any_propagator.Queue.push propagation_queue constraint_propagator c
    in
    { Domains_intf.evt_atomic_change
    ; evt_composite_change
    ; evt_watch_trigger
    }
  in
  ...
  let bitlist_events =
    events
      bitlist_constraint_propagator
      bitlist_structural_propagator
      interval_from_bitlist_propagator
  and interval_events =
    events
      interval_constraint_propagator
      interval_structural_propagator
      bitlist_from_interval_propagator
  in
  ...
  { to_simplify ; propagation_queue ; consistency_queue },
  { intervals = idom
  ; intervals_uf = uf_idom
  ; interval_variables = ivars
  ; interval_changed = HX.create 17
  ; bitlists = bdom
  ; bitlists_uf = uf_bdom
...
let rec propagate_all uf eqs bdom idom =
  ...
  if do_any then
    let queues, state = state uf idom bdom in
    ...
    in
    let eqs, (bdom, idom) =
      simplify_all uf eqs queues.to_simplify (bdom, idom)
    in
...
type t =
  { terms : SX.t
  ; size_splits : Q.t
  ; poly : Expr.t IntPolynomial.Map.t }

let empty uf =
  { terms = SX.empty
  ; size_splits = Q.one
  ; poly = IntPolynomial.Map.empty },
...
let assume env uf la =
  ...
  let result =
    ...
    ; remove = [] }
  in
  { size_splits ; terms = env.terms ; poly = env.poly },
...
let add env uf r t =
  ...
  | Tint -> (
      ...
      | { f = Op BV2Nat ; xs = [ x ] ; _ } ->
        ...
        let eqs =
          if X.equal (Shostak.Arith.is_mine px) r then
            eqs
          else
            let k =
              BV2Nat.T.Ints.of_repr @@
              Interner.find_bv2poly global_interner x
            in
            let lit =
              BV2Nat.T.Ints.mkv_eq (Shostak.Arith.embed r)
                BV2Nat.T.Ints.(px + k *$$ Z.(one lsl sz))
            in
            (lit, exp) :: eqs in
        env,
  ...
  | Tbitv _ -> (
      ...
      | _ ->
        let eqs, poly =
          (* Record equality with any existing term that normalizes to the same
             bit-vector polynomial. *)
          let bp = bv2poly uf t in
          match find_poly uf env bp with
          | r', ex' ->
            let r, ex = Uf.find uf t in
            if X.equal r r' then [], env.poly
            else
              let lit =
                BV2Nat.T.BV.mkv_eq
                  (r, BV2Nat.Extraction.full ~size:(bitwidth r))
                  (r', BV2Nat.Extraction.full ~size:(bitwidth r'))
              in
              [(lit, Ex.union ex ex')], env.poly
          | exception Not_found ->
            [], IntPolynomial.Map.add bp t env.poly
        in
