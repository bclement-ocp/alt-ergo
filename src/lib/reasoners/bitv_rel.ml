...
module ExplainedComparable(V : Rel_utils.ComparableType) :
  Rel_utils.ComparableType with type t = V.t explained =
struct
...
type state =
  { intervals_uf : Interval_domains_uf.t
  ; bitlists_uf : Bitlist_domains_uf.t
  ; intervals : Interval_domains.Ephemeral.t
  ; bitlists : Bitlist_domains.Ephemeral.t
  ; interval_variables : XAtom.Set.t XAtom.Map.t
  ; bitlist_variables : XAtom.Set.t XAtom.Map.t
...
let interval_structural_propagator =
  ...
  let open
    Rel_utils.HandleNotations(Interval_domain)(Interval_domains_uf)
  in
  let get r = !!(Interval_domains_uf.entry dom r) in
  let update r d =
